...
from minigpt4.datasets.datasets.pmcvqa_dataset import PMCVQADataset
from minigpt4.datasets.datasets.slake_dataset import SlakeDataset
...
class VQARADBuilder(DocumentVQABuilder):
    train_dataset_cls = VQARADDataset
    DATASET_CONFIG_DICT = {"default": "configs/datasets/vqarad/defaults.yaml"}
    ...
    def build_datasets(self):
        ...
        dataset_cls = self.train_dataset_cls
        datasets['train'] = dataset_cls(
            vis_processor=self.vis_processors,
            text_processor=self.text_processors,
            ann_path=build_info.ann_path,
            vis_root=build_info.image_path,
        )
...
class PMCVQABuilder(DocumentVQABuilder):
    train_dataset_cls = PMCVQADataset
    DATASET_CONFIG_DICT = {"default": "configs/datasets/pmcvqa/defaults.yaml"}
...
class SlakeBuilder(DocumentVQABuilder):
    train_dataset_cls = SlakeDataset
    DATASET_CONFIG_DICT = {"default": "configs/datasets/slake/defaults.yaml"}
